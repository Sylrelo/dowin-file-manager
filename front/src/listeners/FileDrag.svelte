<script lang="ts">
    ...
    $: $mouseEvent &&
        ($currentDrag === CurrentDrag.None ||
            $currentDrag === CurrentDrag.File) &&
        detectDragStart();
    ...
    const detectDragStart = () => {
        ...
        if (
            null != startPosition &&
            startPosition.sub(tmpMousePos).lenght >= 30
        ) {
    ...
    const handleDragRelease = (action: string) => {
        ...
        const dstPath =
            targetOnRelease.file?.path ?? targetOnRelease.content?.path;
        if (null == dstPath) {
        ...
        fsJobQueue.addMultiple(action as FsJobType, srcs, dstPath);
    };
