<script lang="ts">
    ...
    import { CurrentDrag, activeWindow, currentDrag } from "../stores/global";

    $: if ($mouseButtonEvent.primary && mouseButtonEvent.value.target?.window) {
        activeWindow.set(mouseButtonEvent.value.target.window.uuid);
    }
    ...
    $: if (
        ...
        winTarget.style.top =
            +winTarget.style.top.slice(0, -2) +
            $mouseEvent.movement.y * 0.8 +
            "px";

        winTarget.style.left =
            +winTarget.style.left.slice(0, -2) +
            $mouseEvent.movement.x * 0.8 +
            "px";
    }
